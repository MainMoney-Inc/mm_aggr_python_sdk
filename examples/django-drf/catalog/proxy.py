...
from catalog.models import Order, Transfer
from catalog.sessions import CheckoutSession
...
def _mark_order(session: CheckoutSession, status: str) -> None:
    ...
    Order.objects.filter(pk=session.order_id).update(status=mapped)
...
def _mark_transfer(session: CheckoutSession, status: str, destination: str = "") -> None:
    if session.transfer_id is None:
        return
    mapped = _map_status(status)
    updates: dict[str, str] = {"status": mapped}
    if destination:
        updates["destination"] = destination
    Transfer.objects.filter(pk=session.transfer_id).update(**updates)
